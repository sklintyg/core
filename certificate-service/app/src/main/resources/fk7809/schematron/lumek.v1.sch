...
<iso:schema
  ...
  <iso:title>Schematron file for "Läkarutlåtande för merkostnadsersättning FK7809" - Version 1.</iso:title>
  ...
  <iso:pattern id="intyg">
    <iso:rule context="//rg:intyg">
      ...
      <iso:assert test="count(gn:svar[@id='3']) = 1">
        Ett 'LUMEK' måste ha ett 'Är utlåtandet även baserat på andra medicinska utredningar eller underlag?' svar
      </iso:assert>
      <iso:assert test="count(gn:svar[@id='4']) le 3">
        Ett 'LUMEK' får ha högst tre 'Andra medicinska utredningar eller underlag'
      </iso:assert>
      <iso:assert test="count(gn:svar[@id='58']) = 1">
        Ett 'LUMEK' måste ha en 'Diagnos'
     </iso:assert>
  ...
  <iso:pattern id="q1.1">
    <iso:rule context="//gn:delsvar[@id='1.1']">
      ...
      <iso:assert test="matches(normalize-space(tp:cv/tp:code), '^(UNDERSOKNING|JOURNALUPPGIFTER|ANNAT|ANHORIG)$')">
  ...
  <iso:pattern id="q1.4">
      <iso:rule context="//gn:delsvar[@id='1.4']">
        <iso:extends rule="non-empty-string"/>
        <iso:assert test="count(../gn:delsvar[@id='1.1']/tp:cv/tp:code[normalize-space(.) != 'ANHORIG']) = 0">
          Om 'Typ av grund för MU' inte är 'Anhörig' så får 'Ange anhörig eller annas relation till patienten' inte anges.
        </iso:assert>
      </iso:rule>
    </iso:pattern>
    ...
    <iso:pattern id="q3">
        <iso:rule context="//gn:svar[@id='3']">
          <iso:assert test="count(gn:delsvar[@id='3.1']) = 1">
            'Är utlåtandet även baserat på andra medicinska utredningar eller underlag?' måste ha ett delsvar.
          </iso:assert>
        ...
        <iso:pattern id="q3.1-4">
          ...
          <iso:rule context="//gn:delsvar[@id='3.1' and (normalize-space(.)='0' or normalize-space(.)='false')]">
            <iso:assert test="count(../../gn:svar[@id='4']) = 0">
              Om 'Är utlåtandet även baserat på andra medicinska utredningar eller underlag?' besvarats med falskt får 'Andra medicinska utredningar eller underlag' inte anges.
            </iso:assert>
          </iso:rule>
        </iso:pattern>
        ...
        Oväntat delsvars-id i delsvar till svar "<value-of select="@id"/>". Delsvars-id:n måste matcha "<value-of select="$delsvarsIdExpr"/>".
          ...
          <iso:pattern id="q4.1">
            <iso:rule context="//gn:delsvar[@id='4.1']">
              ...
              <iso:assert test="matches(normalize-space(tp:cv/tp:code), '^(NEUROPSYKIATRISKT|HABILITERING|ARBETSTERAPEUT|FYSIOTERAPEUT|LOGOPED|PSYKOLOG|SPECIALISTKLINIK|VARD_UTOMLANDS|HORSELHABILITERING|SYNHABILITERING|AUDIONOM|DIETIST|ORTOPTIST|ORTOPEDTEKNIKER|OVRIGT|)$')">
            'Utredning eller underlagstyp?' kan ha ett av värdena NEUROPSYKIATRISKT, HABILITERING, ARBETSTERAPEUT, FYSIOTERAPEUT, LOGOPED, PSYKOLOG, SPECIALISTKLINIK, VARD_UTOMLANDS, HORSELHABILITERING, SYNHABILITERING, AUDIONOM, DIETIST, ORTOPTIST, ORTOPEDTEKNIKER, OVRIGT.
              </iso:assert>
            </iso:rule>
          </iso:pattern>
  ...
  <iso:pattern id="q58">
    ...
    <iso:pattern id="q58.3">
      <iso:rule context="//gn:delsvar[@id='58.3']">
        <iso:extends rule="non-empty-string"/>
      </iso:rule>
    </iso:pattern>
    ...
    <iso:pattern id="q58.3-6.4">
      <iso:rule context="//gn:svar[@id='58']">
        <iso:assert test="(count(gn:delsvar[@id='58.3']) = 1 and count(gn:delsvar[@id='58.4']) = 1) or (count(gn:delsvar[@id='58.3']) = 0 and count(gn:delsvar[@id='58.4']) = 0)">
          Både 'Diagnoskod' och 'Diagnostext' måste finnas för en bidiagnos.
        </iso:assert>
      </iso:rule>
    </iso:pattern>

    <iso:pattern id="q58.5-6.6-6.3">
      <iso:rule context="//gn:svar[@id='58']">
    ...
    <iso:pattern id="q58.2-6.4-6.6-R12">
      <iso:rule context="//gn:svar[@id='58']">
        <iso:assert test="(matches(normalize-space(gn:delsvar[@id='58.2']/tp:cv/tp:codeSystem), normalize-space(gn:delsvar[@id='58.4']/tp:cv/tp:codeSystem)) and matches(normalize-space(gn:delsvar[@id='58.2']/tp:cv/tp:codeSystem), normalize-space(gn:delsvar[@id='58.6']/tp:cv/tp:codeSystem)))">
          'Huvuddiagnoskod' (DFR 58.2), 'Bidiagnoskod 1' (DFR 58.4) och 'Bidiagnoskod 2' (DFR 58.6) ska besvaras med samma kodverk.
        </iso:assert>
      </iso:rule>
    </iso:pattern>
