...
<iso:schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:iso="http://purl.oclc.org/dsdl/schematron"
  ...
  <iso:pattern id="intyg">
    <iso:rule context="//rg:intyg">
      <iso:assert test="count(gn:svar[@id='1']) le 5">Ett 'LISJP' får ha högst 5 'Grund för medicinskt underlag'</iso:assert>
      <iso:assert test="count(gn:svar[@id='28']) le 4">Ett 'LISJP' får ha högst 4 'Typ av sysselsättning'</iso:assert>
      <iso:assert test="count(gn:svar[@id='29']) le 1">Ett 'LISJP' får ha högst ett 'Nuvarande arbete'</iso:assert>
      <iso:assert test="count(gn:svar[@id='6']) le 3">Ett 'LISJP' får ha högst 3 'Diagnos'</iso:assert>
      <iso:assert test="count(gn:svar[@id='19']) le 1">Ett 'LISJP' får ha högst ett 'Pågående medicinska behandlingar'</iso:assert>
      <iso:assert test="count(gn:svar[@id='32']) le 4">Ett 'LISJP' får ha högst 4 'Bedömning'</iso:assert>
      <iso:assert test="count(gn:svar[@id='37']) le 1">Ett 'LISJP' får ha högst ett 'Försäkringsmedicinskt beslutsstöd'</iso:assert>
      <iso:assert test="count(gn:svar[@id='33']) le 1">Ett 'LISJP' får ha högst ett 'Arbetstidsförläggning'</iso:assert>
      <iso:assert test="count(gn:svar[@id='34']) le 1">Ett 'LISJP' får ha högst ett 'Arbetsresor'</iso:assert>
      <iso:assert test="count(gn:svar[@id='39']) le 1">Ett 'LISJP' får ha högst ett 'Prognos'</iso:assert>
      <iso:assert test="count(gn:svar[@id='44']) le 1">Ett 'LISJP' får ha högst ett 'Arbetslivsinriktade åtgärder aktuellt'</iso:assert>
      <iso:assert test="count(gn:svar[@id='25']) le 1">Ett 'LISJP' får ha högst ett 'Övrigt'</iso:assert>
      <iso:assert test="count(gn:svar[@id='26']) le 1">Ett 'LISJP' får ha högst ett 'Kontakt önskas'</iso:assert>
      <iso:assert test="count(gn:svar[@id='27']) le 1">Ett 'LISJP' får ha högst ett 'Smittbärarpenning'</iso:assert>
  ...
  <iso:pattern id="q1.1">
    <iso:rule context="//gn:delsvar[@id='1.1']">
      ...
      <iso:assert test="matches(normalize-space(tp:cv/tp:code), '^(FYSISKUNDERSOKNING|DIGITALUNDERSOKNING|TELEFONKONTAKT|JOURNALUPPGIFTER|ANNAT)$')">
        'Typ av grund för MU' kan ha ett av värdena FYSISKUNDERSOKNING, DIGITALUNDERSOKNING, TELEFONKONTAKT, JOURNALUPPGIFTER, ANNAT.
      </iso:assert>
    </iso:rule>
  </iso:pattern>
  ...
  <iso:pattern id="q27.1">
    <iso:rule context="//gn:delsvar[@id='27.1']">
      ...
      <iso:assert test="matches(normalize-space(.), '1|true') or count(//gn:svar[@id='1']) ge 1">
        Ett 'LISJP' måste ange minst en 'Grund för medicinskt underlag' om 'Om smittbärarpenning' är 'false'.
      </iso:assert>
      <iso:assert test="matches(normalize-space(.), '1|true') or count(//gn:svar[@id='28']) ge 1">
        Ett 'LISJP' måste ange minst en 'Typ av sysselsättning' om 'Om smittbärarpenning' är 'false'.
      </iso:assert>
  ...
  <iso:pattern id="q6">
    <iso:rule context="//gn:svar[@id='6']">
      <iso:assert test="count(gn:delsvar[@id='6.1']) = 1">'Diagnos' måste ha ett 'Huvuddiagnostext'.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='6.2']) = 1">'Diagnos' måste ha ett 'Huvuddiagnoskod'.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='6.3']) le 1">'Diagnos' får ha högst ett 'Bidiagnostext'.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='6.4']) le 1">'Diagnos' får ha högst ett 'Bidiagnoskod'.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='6.5']) le 1">'Diagnos' får ha högst ett 'Bidiagnostext'.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='6.6']) le 1">'Diagnos' får ha högst ett 'Bidiagnoskod'.</iso:assert>
  ...
  <iso:pattern id="q19">
    <iso:rule context="//gn:svar[@id='19']">
      <iso:assert test="count(gn:delsvar[@id='19.1']) = 1">'Medicinsk behandling' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q25">
    <iso:rule context="//gn:svar[@id='25']">
      <iso:assert test="count(gn:delsvar[@id='25.1']) = 1">'Övrigt' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q27">
    <iso:rule context="//gn:svar[@id='27']">
      <iso:assert test="count(gn:delsvar[@id='27.1']) = 1">'Smittbärarpenning' måste ha ett booleskt delsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q28">
    <iso:rule context="//gn:svar[@id='28']">
      <iso:assert test="count(gn:delsvar[@id='28.1']) = 1">'Sysselsättning' måste ha ett kodverksdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q29">
    <iso:rule context="//gn:svar[@id='29']">
      <iso:assert test="count(gn:delsvar[@id='29.1']) = 1">'Nuvarande arbete' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q32">
    <iso:rule context="//gn:svar[@id='32']">
      <iso:assert test="count(gn:delsvar[@id='32.1']) = 1">'Nedsättning av arbetsförmågan' måste ha ett kodverksdelsvar.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='32.2']) = 1">'Nedsättning av arbetsförmågan' måste ha ett datumdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q33">
    <iso:rule context="//gn:svar[@id='33']">
      <iso:assert test="count(gn:delsvar[@id='33.1']) = 1">'Arbetstidsförläggning' måste ha ett booleskt delsvar.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='33.2']) = 1">'Arbetstidsförläggning' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q34">
    <iso:rule context="//gn:svar[@id='34']">
      <iso:assert test="count(gn:delsvar[@id='34.1']) = 1">'Arbetsresor' måste ha ett booleskt delsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q37">
    <iso:rule context="//gn:svar[@id='37']">
      <iso:assert test="count(gn:delsvar[@id='37.1']) = 1">'Försäkringsmedicinskt beslutsstöd' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q39">
    <iso:rule context="//gn:svar[@id='39']">
      <iso:assert test="count(gn:delsvar[@id='39.1']) = 1">'Prognos' måste ha ett kodverksdelsvar.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='39.2']) le 1">'Prognos' får ha högst ett textdelsvar.</iso:assert>
      <iso:assert test="count(gn:delsvar[@id='39.4']) le 1">'Prognos' får ha högst ett textdelsvar för antal månader.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="q44">
    <iso:rule context="//gn:svar[@id='44']">
      <iso:assert test="count(gn:delsvar[@id='44.1']) = 1">'Åtgärder som kan främja återgången i arbete' måste ha ett textdelsvar.</iso:assert>
    </iso:rule>
  </iso:pattern>
